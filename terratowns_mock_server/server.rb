...
# by setting a global variable, you would nver use a global variable
# in a production server
$home = {}

# This is a ruby class that include validations for ActiveRecord
...
class Home
  ...
  # lock this down to only be from cloud front
  validates :domain_name, 
    format: { with: /\.cloudfront\.net\z/, message: "domain must be from .cloudfront.net" }
    # uniqueness: true, 

  # content version has to be an integer
  # make sure it is an incemental version in the controller i.e., content_version=1, content_version=2
...
class TerraTownsMockServer < Sinatra::Base
  ...
  def find_user_by_bearer_token
    ...
    # return an error if token is not found or does not match
    ...
    # in rails: User.find_by_access_code: code, user_uuid: user_uuid
  ...
  post '/api/u/:user_uuid/homes' do
    ensure_correct_headings() # ruby functions don't require () - () are included here to denote functions
